<template>
    <div class="dbFile">
        <el-table v-loading="loading" :data="tableData" size="mini" align="center" style="width: auto">
            <el-table-column type="index" label="序号" align="center" width="50">
                <template slot-scope="scope">
                    <section>{{getRowIndex(scope.$index)}}</section>
                </template>
            </el-table-column>
            <el-table-column prop="name" label="文件名" align="center" header-align="center" min-width="150"/>
            <el-table-column prop="version" label="版本号" align="center" header-align="center" min-width="60"/>
            <el-table-column label="文件大小M" align="center" header-align="center">
                <template slot-scope="scope">
                    <span>{{ scope.row.size | formatFileSize(3) }}</span>
                </template>
            </el-table-column>
            <el-table-column prop="creator" label="创建者" align="center" header-align="center"/>
            <el-table-column prop="createTime" label="创建时间" align="center" header-align="center"  min-width="120"/>
            <el-table-column prop="modifier" label="修改者" align="center" header-align="center"/>
            <el-table-column prop="modifyTime" label="修改时间" align="center" header-align="center" min-width="120"/>
            <el-table-column label="操作"  align="center" header-align="center" width="200">
                <template slot-scope="scope">
                    <el-button round size="mini"  @click="handleEdit(scope.row.id,scope.row.userId)">编辑/预览</el-button>
                    <el-button round  size="mini" slot="reference" @click="handleDel(scope.row.id)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
        <pagination v-if="showOther" :total="total" :page.sync="pageR.page" :limit.sync="pageR.size" @pagination="getFileList"/>
        <el-upload v-if="showOther" style="float: right" :action="action" :show-file-list="false" :before-upload="beforeUpload"
            :on-progress="handleProgress" :on-success="handleAvatarSuccess" :on-change="handleChange" :on-error="errFunc">
            ...
            <el-dropdown-menu slot="dropdown">
                <el-dropdown-item command="word">
                    <svg class="icon" aria-hidden="true">
                        <use xlink:href="#icon-word1"/>
                    </svg>
                    <span style="text-align: center">文字文档</span>
                </el-dropdown-item>
